...
const EditPersonal = () => {
     ...
     useEffect(()=>{
        const getProductById = async () =>{
            const response = await axios.get(`http://localhost:5000/personal/${idpersonal}`);
            setCedula(response.data.cedula);
            setNombre(response.data.nombre);
            setApellido_1(response.data.apellido_1);
            setApellido_2(response.data.apellido_2);
     ...
     const updatePersonal = async (e) => {
        e.preventDefault();
        await axios.patch(`http://localhost:5000/personal/${idpersonal}`, {
            cedula: cedula,
            nombre: nombre,
  ...
  return (
    <div className='max-w-lg mx-auto my-10 bg-white p-8 rounded-xl shadow shadow-slate-300'>
        <form onSubmit={updatePersonal} className='my-10'>
